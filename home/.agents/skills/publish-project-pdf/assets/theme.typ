// Starter theme for project-local Pandoc and Typst PDF builds.
// Copy and adapt this file; do not import it from the installed skill.
...
#let document-label = [Project documentation]
#let cover-label = [PROJECT DOCUMENTATION]
#let cover-detail = [Technical documentation]
...
#let conf(
  ...
  let project-title = if title == none {
    [Untitled project]
  } else {
    title
  }
  ...
  page(
    ...
    #v(8pt)
    #text(size: 34pt, weight: "bold", fill: white)[#project-title]
    #v(1fr)
    #text(size: 9pt, fill: rgb("#aebbd0"))[#cover-detail]
  ]
  ...
  set page(
    ...
    header: {
      ...
      grid(
        columns: (1fr, auto),
        align: (left, right),
        text(weight: "semibold", tracking: 0.06em)[#project-title],
        document-label,
      )
      v(3pt)
